...
function EditarCertificacaoComponent({ certificacao, onClose, onSuccess }) {
    const { user } = useAuth();
    ...
    const initialForm = {
        ...certificacao,
        startDate: formatDateToInput(certificacao.startDate),
        endDate: formatDateToInput(certificacao.endDate)
    };
    ...
    const handleSubmit = async (e) => {
        ...
        // Remove ID e FuncionarioCerts do body
        const { id, FuncionarioCerts, ...certificacaoSemIds } = formComDatasFormatadas;
        // console.log(certificacaoSemIds);
        const body = {
            email: user.mail,
            certificacao: certificacaoSemIds,
        };
        console.log(body);
        const confirmado = window.confirm(
            `Solicitante: ${body.email}\n` +
            `Deseja editar a certificação:\n` +
            `Nome: ${form.nome}\n`
        );
        if (confirmado) {
            try {
                const response = await apiServiceCertificacoes.editarCertificacao(body, form.id);
                if (response.status === 200) {
                    alert("Certificação editada com sucesso!");
                    if (onSuccess) {
                        onSuccess();
                    }
                }
...
export default EditarCertificacaoComponent;
