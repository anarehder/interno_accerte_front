...
export async function getSharePointData(instance, accounts) {
  ...
  const beneficios = await fetch(
    `https://graph.microsoft.com/v1.0/drives/${sharedDocumentsId}/root:/Extras/BENEFICIOS:/children`,
    {
      headers: { Authorization: `Bearer ${response.accessToken}` }
    }
  );
  const files14 = await beneficios.json();

  const fileList14 = files14.value
  .map(file => ({
    name: file.name,
    url: file.webUrl
  }))
  ...
  const accerteconnect3 = await fetch(
    `https://graph.microsoft.com/v1.0/drives/${sharedDocumentsId}/root:/Extras/ACCERTE CONNECT 2026.1:/children`,
    {
      headers: { Authorization: `Bearer ${response.accessToken}` }
    }
  );
  const files16 = await accerteconnect3.json();

  const fileList16 = files16.value.map(file => ({
    name: file.name,
    url: file.webUrl
  }));
